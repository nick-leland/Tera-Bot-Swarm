...
if __name__ == "__main__":
    # --- Tunables ---
    SIM_THRESHOLD = 0.995    # similarity to consider "no change"
    CONSEC_REQUIRED = 5        # how many consecutive "no change" frames to declare clamp
    STEP_DY = -1       # one "interception pixel" per step (downward)
    FRAME_DELAY = 0.50     # seconds between input and screenshot
    WRITE_WAIT = 0.55     # after printscreen, let Windows write file

    TERA_SS_DIR = r"C:\Users\chaot\Pictures\TERA_ScreenShots"
    ...
    # Snap roughly to top, HUD off for stability
    interception.move_relative(0, +90000)   # shove up
    toggle_hud(mode=True)

    # Measure top→bottom range in counts ("interception pixels")
    pitch_range_counts = measure_pitch_range_counts(_latest_path_and_image, CONSEC_REQUIRED, FRAME_DELAY, WRITE_WAIT, SIM_THRESHOLD)

    print(f"Total yaw range of {pitch_range_counts} interception pixels")
